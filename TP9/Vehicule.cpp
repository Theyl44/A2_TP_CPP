...
//DIESEL VEHICULE
//---------------------------------------
DieselVehicle :: DieselVehicle(DieselVehicle & p){
    std::cout << "\tCrea DieselVehicule(Copie)"<<std::endl;
    this->consommation = p.consommation;
    this->carburant = p.carburant;
    this->filtreAParticules = p.filtreAParticules;
...
DieselVehicle :: ~DieselVehicle(){
    std::cout << "\tDestruction du DieselVehicule()"<<std::endl;
}
//---------------------------------------
//GAS VEHICULE
//---------------------------------------

GasVehicle :: GasVehicle(Point p, char* name, float carburant, float consommation,int octaneRate){
...
void GasVehicle :: afficher(){
    ...
    std::cout<<"position : ";
    getPosition().afficherP();
    std::cout<<", consumption: "<< this->consommation<<", fuel left: "<< this->carburant<< ", octane rating: " << this->octaneRate <<endl;

}
GasVehicle :: GasVehicle(GasVehicle &p){
    std::cout << "\tCrea GasVehicule(Copie)"<<std::endl;
    this->consommation = p.consommation;
    this->carburant = p.carburant;
    this->octaneRate = p.octaneRate;
    this->setId(p.getId());
    this->setPosition(p.getPosition());
}
GasVehicle :: ~GasVehicle(){
    std::cout << "\tDestruction du GasVehicule()"<<std::endl;
}
